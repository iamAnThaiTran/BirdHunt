...
#include <fstream>
using namespace std;
void van_toc(float x0, float y0, float x1, float y1, float &x, float &y, float hangso)
{
	float canh_huyen = sqrt((x1 - x0) *(x1 - x0) + (y1 - y0) *(y1 - y0));
	float cos = (x1 - x0) / canh_huyen;
	float sin = (y1 - y0) / canh_huyen;
	x = hangso * cos;
	y = hangso * sin;
...
class Base
{
	...
	int hp, bien_duoi = 475, bien_ngang_left = 0, bien_ngang_right = 1200;
...
TextObject Score;
vector<TextObject> list_chi_so_hp;
...
void Bullet::BulletMove()
{
	...
	{
		if (mPosx > 1200 || mPosx < 0 || mPosy < 0 || mPosy > bien_duoi)
		{
			ismove = false;
		}
...
void MainObject::HandleAction(SDL_Event & e)
{
	...
	if (e.type == SDL_KEYDOWN && e.key.repeat == 0)
	{
		...
		else if (e.key.keysym.sym == SDLK_UP && SDL_GetTicks() - time_dinh_choang >= 5000)
		{
			mPosy -= 150;
		}
	}
	...
	else if (e.type == SDL_MOUSEBUTTONDOWN)
	{
		...
		van_toc(mPosx, mPosy, x1, y1, x, y, 20);
		new_bul.SetVel(x, y);
...
/*ClASS Threast chim choc muong thu cac kieu */
class Threat: public Base
{
	...
	bool muc_tieu_bom = false;
	bool chuyen_huong = false;
	void set_muc_tieu_bom(float x, float y)
	{
		vel_bom_x = x;
		vel_bom_y = y;
	}
...
void Threat::chim_bom_move()
{
	if (move_left_or_right == false)
	{
		if (mPosx < 0 || mPosy > bien_duoi || CheckCollision(human.rect, rect))
		{
			Hieu_ung new_hieu_ung;
			if (human.get_pos_x() >= mPosx - 50 - 50 && human.get_pos_y() >= mPosy - 50 - 50 && human.get_pos_x() <= mPosx - 50 + 240 && human.get_pos_y() <= mPosy - 50 + 150)

			{

				human.hp -= 30;
				TextObject new_chi_so_hp;
										new_chi_so_hp.Set_Color(1);
										new_chi_so_hp.Set_Text("HP -- " + doi_so_sang_text(30));
										new_chi_so_hp.setrect(human.get_pos_x(), human.get_pos_y() - 40);
										new_chi_so_hp.loadFromRenderedText();
										new_chi_so_hp.time_text = SDL_GetTicks();
										//new_chi_so_hp.render();
										list_chi_so_hp.push_back(new_chi_so_hp);
			}

			new_hieu_ung.setrect(mPosx - 70, mPosy - 70);
			new_hieu_ung.loadfromfile("exp1.png");
	...
	{
		if (mPosx > 1200 || mPosy > bien_duoi || CheckCollision(human.rect, rect))
		{
			Hieu_ung new_hieu_ung;
			if (human.get_pos_x() >= mPosx - 50 - 50 && human.get_pos_y() >= mPosy - 50 - 50 && human.get_pos_x() <= mPosx - 50 + 240 && human.get_pos_y() <= mPosy - 50 + 150)
			{
...
Base pause, new_game, resume, continue_game;
int map_index[20][10];
TextObject text_level, text_hp, text_task, text_dan, chi_so_hp, chi_so_dan;
	// text
	...
	while (quit != true)
	{
		while (SDL_PollEvent(&e) != 0)
		{
			...
			if (vao_game == true)
			{
				if (pause_game == false)
				{
					if(level <10) human.HandleAction(e);
				...
				else
				{
					if (e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEMOTION)
					{
						int x, y;
						SDL_GetMouseState(&x, &y);
						if (300 <= x && 900 >= x && 23 <= y && 340 >= y && e.type == SDL_MOUSEBUTTONDOWN)
						{
							continue_game.trang_thai_bat_tat = 0;
							pause_game = false;
						}

						if (420 <= x && 770 >= x && 120 <= y && 196 >= y && e.type == SDL_MOUSEMOTION)
						{
							continue_game.trang_thai_bat_tat = 1;
						}
						else continue_game.trang_thai_bat_tat = 0;
					}
				}
			}
		...
		//cout << human.time_dinh_choang << endl ;
		if (SDL_GetTicks() - human.time_dinh_choang >= 5000)
		{;
			human.HandleMove();
		}
		...
		{ /*chua vao game, menu start */
			...
			//cout << human.time_dinh_choang <<"herre" <<  endl ;
			...
			//cout << human.time_dinh_choang << "    123"<< endl ;
			//human.time_dinh_choang = 999 ;
		...
		{ /*neu pause game */
		    if(level >= 10)
            {
                background.loadfromfile("back_ground_buoi_chieu_win.png") ;
                            background.setrect(0,0) ;
                            background.render() ;
            }
			else if (pause_game == true)
			{ /*menu pause game */
				...
				if (continue_game.trang_thai_bat_tat == 0)
					continue_game.loadfromfile("continue1.png");
				else continue_game.loadfromfile("continue2.png");
				continue_game.setrect(300, 23);
			...
			{ /*neu win levle */
				...
				{ /*choi_game */
					...
					else human.bien_duoi = 415;

					if (map_index[x - 1][y] == 1)
					{
						human.bien_ngang_left = 60 *(x);
					}
					else
					{
						human.bien_ngang_left = 0;
					}

					if (map_index[x + 1][y] == 1)
					{
						human.bien_ngang_right = 60 *(x + 1);
					}
					else
					{
						human.bien_ngang_right = 1200;
					}
					...
					/*xu ly chim + di chuyen + bullet cua chim, chim BOOOOOMMMMMMM */
					for (int i = 0; i < so_luong_chim; i++)
					{
						threat[i].loadfromfile(chim_at_level[i]);
						if (chim_at_level[i] == "chim_bom.png" && threat[i].chuyen_huong == true)
						{
							if (threat[i].muc_tieu_bom == false)
							{
								float x = 0, y = 0;
								van_toc(threat[i].get_pos_x(), threat[i].get_pos_y(), human.get_pos_x(), human.get_pos_y(), x, y, 10);
								threat[i].set_muc_tieu_bom(x, y);
								threat[i].muc_tieu_bom = true;
							}

							threat[i].chim_bom_move();
							if (CheckCollision(human.rect, threat[i].rect))
							{
								threat[i].hp = 0;
							}
						}
						...
						for (int j = 0; j < threat[i].Bul.size(); j++)
						{
							...
							if (threat[i].Bul[j].da_lay_muctieu == false)
							{
								...
								float x = 0, y = 0;
								van_toc(threat[i].Bul[j].get_pos_x(), threat[i].Bul[j].get_pos_y(), human.get_pos_x(), human.get_pos_y(), x, y, 10);
								threat[i].Bul[j].SetVel(x, y);
							...
							if (CheckCollision1(human, threat[i].Bul[j]) == true || threat[i].Bul[j].get_move() == false)
							{
								...
								{
								    if(chim_at_level[i] == "pig_fly.png")
                                    {
                                        human.hp = 0 ;
                                    }
                                    else
									...
									new_chi_so_hp.Set_Color(1);
									new_chi_so_hp.Set_Text("HP --" + doi_so_sang_text(10));
									new_chi_so_hp.setrect(human.get_pos_x(), human.get_pos_y() - 20);
									new_chi_so_hp.loadFromRenderedText();
									new_chi_so_hp.time_text = SDL_GetTicks();
									//new_chi_so_hp.render();
									list_chi_so_hp.push_back(new_chi_so_hp);
									if (chim_at_level[i] == "chim_vang.png")
									{
										human.time_dinh_choang = SDL_GetTicks();
										TextObject new_chi_so_hp;
										new_chi_so_hp.Set_Color(1);
										new_chi_so_hp.Set_Text("Troi Chan " + doi_so_sang_text(5) + " giay");
										new_chi_so_hp.setrect(human.get_pos_x(), human.get_pos_y() - 60);
										new_chi_so_hp.loadFromRenderedText();
										new_chi_so_hp.time_text = SDL_GetTicks();
										//new_chi_so_hp.render();
										list_chi_so_hp.push_back(new_chi_so_hp);
					...
					for (int k = 0; k < human.Bul.size(); k++)
					{
						...
						{
							if (CheckCollision(threat[i].rect, human.Bul[k].rect))
							{
								human.Bul.erase(human.Bul.begin() + k);
								threat[i].hp -= human.Bul[k].hp;
								if (threat[i].hp == 0)
								{
									if (chim_at_level[i] == "chim_xanh.png")
									{
										Item new_item;
										new_item.ten_item = 4;
										...
										int x = lay_ra_o(new_item.get_pos_x());

										if (map_index[x][8] == 0)
										{
											new_item.item_bien = 475;
										}
										else new_item.item_bien = 425;
									...
									if (chim_at_level[i] == "chim_bom.png" && (human.get_pos_x() >= threat[i].get_pos_x() - 50 - 50 && human.get_pos_y() >= threat[i].get_pos_y() - 50 - 50 && human.get_pos_x() <= threat[i].get_pos_x() - 50 + 240 && human.get_pos_y() <= threat[i].get_pos_y() - 50 + 160))
									{
										human.hp -= 30;
										TextObject new_chi_so_hp;
										new_chi_so_hp.Set_Color(1);
					...
					{ /*thoi gian ton tai cua item */
						...
						switch (list_item[li].ten_item)
						{
							...
							case 3:
								...
								break;
							case 4:
								list_item[li].loadfromfile("axit.png");
								break;
						}

						if (SDL_GetTicks() - list_item[li].time_ton_tai >= 8000.0 && list_item[li].ten_item != 4)
						{
							list_item.erase(list_item.begin() + li);
							//continue ;
						}

						if (CheckCollision(list_item[li].rect, human.rect) == true)
						{
							if (list_item[li].ten_item == 1)
								{human.hp += 10;
								TextObject new_chi_so_hp;
										new_chi_so_hp.Set_Color(1);
										new_chi_so_hp.Set_Text("HP + " + doi_so_sang_text(10));
							...
							else if (list_item[li].ten_item == 4) {
                                    human.hp -= 40;
                            TextObject new_chi_so_hp;
										new_chi_so_hp.Set_Color(1);
										new_chi_so_hp.Set_Text("HP -- " + doi_so_sang_text(40));
										new_chi_so_hp.setrect(human.get_pos_x(), human.get_pos_y() - 40);
										new_chi_so_hp.loadFromRenderedText();
										new_chi_so_hp.time_text = SDL_GetTicks();
										//new_chi_so_hp.render();
							...
							else {so_luong_dan += 10;
							TextObject new_chi_so_hp;
										new_chi_so_hp.Set_Color(1);
										new_chi_so_hp.Set_Text("Bullet + " + doi_so_sang_text(10));
										new_chi_so_hp.setrect(human.get_pos_x(), human.get_pos_y() - 40);
										new_chi_so_hp.loadFromRenderedText();
										new_chi_so_hp.time_text = SDL_GetTicks();
										//new_chi_so_hp.render();
						...
						//cout << list_item[li].rect.x << " " << list_item[li].rect.y << " " << list_item[li].rect.w << " " << list_item[li].rect.h<< endl ;
						//cout << human.rect.x << " " << human.rect.y << " " << human.rect.w << " " << human.rect.h<< endl ;
						list_item[li].render();
					}

					for (int ji = 0; ji < list_chi_so_hp.size(); ji++)
					{
						if (SDL_GetTicks() - list_chi_so_hp[ji].time_text <= 1000)
							{
							    list_chi_so_hp[ji].loadFromRenderedText();list_chi_so_hp[ji].render();}


						else list_chi_so_hp.erase(list_chi_so_hp.begin() + ji);
					}

					/*xu ly vu no */
					for (int q = 0; q < list_exp.size(); q++)
					{
						if (list_exp[q].frame == 0)
						{
							Mix_PlayChannel(-1, gBirdYell, 0);
						}

						list_exp[q].loadfromfile("exp1.png");
